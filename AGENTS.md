...
- The onboard scheduler rejects whole chunks on shape/finiteness, session/sequence, quaternion, root displacement/height/speed/yaw-rate, G1 joint position/rate, hand closure, OpenNeck degree range, or staleness failures; the entry candidate exempts only its measured-pose-to-first-frame G1 joint-rate boundary, while root boundary checks and every transition inside the candidate remain mandatory; never pad, trim, or safety-clip invalid host output
