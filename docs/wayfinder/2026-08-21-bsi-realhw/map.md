...
- [真机运行拓扑确认](tickets/06-real-network-topology.md) — 板载 Orin（仓库惯例 + eth0 机器人总线 + Orin cyclonedds 0.10.5 实测 OK）；三机两总线拓扑定稿；5 步上真机验证清单（解码器机对齐 0.10.x 防 XTypes hash 静默丢包 → doctor → 跨机 echo → Orin 起 mp 栈 → 同进程双 DDS 共存）；遗留 run_velocity_sim.py 未接 merged_bsi 待定 fix ticket。
